...
export async function POST(
  ...
  try {
    ...
    let winnerDoc: InstanceType<typeof Winner>[];
    let selectedAttendee: (typeof eligibleAttendees)[number] | null = null;
  ...
  try {
    ...
    if (prizeId === null || prizeId === "") {
      ...
      if (!prize || prize.eventId.toString() !== eventId) {
        return NextResponse.json(
          { success: false, error: "Invalid prize" },
          { status: 400 },
        );
      }
      update.currentDrawPrizeId = prize._id.toString();
    }
...
export async function GET(
  ...
  try {
    ...
    const [winners, event] = await Promise.all([
      Winner.find(query).sort({ wonAt: -1 }).populate("prizeId", "name").lean(),
      Event.findById(eventId)
        .populate<{
          currentDrawPrizeId: { _id: unknown; name: string };
        }>("currentDrawPrizeId", "_id name")
        .lean(),
    ]);
    ...
    const selectedPrizeId = currentPrize
      ? (currentPrize as { _id: unknown })._id?.toString()
      : null;
    const selectedPrizeName =
      currentPrize && typeof currentPrize === "object" && "name" in currentPrize
